!function () {
    ...
    var BONUS_CHANCE = 0.025;

    Game.maps['FourSectors'] = Game.Map.extend({

        build: function () {

            var blockSize = 20;
            var yBlocksCnt = Math.floor(this.game.arenaHeight / blockSize);
            var xBlocksCnt = Math.floor(this.game.arenaWidth / blockSize);
            var xMiddle = this.game.arenaWidth / 2 - blockSize / 2;
            var yMiddle = this.game.arenaHeight / 2 - blockSize / 2;
            for (var i = 0; i < yBlocksCnt; i++) {
                this.game.create('Block', {x: xMiddle, y: blockSize * i});
            }
            for (var i = 0; i < xBlocksCnt; i++) {
                this.game.create('Block', {x: blockSize * i, y: yMiddle});
            }
        ...
        step: function () {
            if (this.game.random() <= BONUS_CHANCE) {
                this.game.create('Bonus', {
                    ...
                    type: this.game.getRandomElement([ 'amanita', 'acorn', 'apple', 'cake', 'pepper'])
                });
            }
        }
